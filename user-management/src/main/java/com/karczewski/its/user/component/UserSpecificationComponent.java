...
public class UserSpecificationComponent {
    ...
    private static void addFilters(final UserFilters filters,
        ...
        addDepartmentPredicateIfProvided(filters, criteriaBuilder, root, predicates);
    ...
    private static void addDepartmentPredicateIfProvided(final UserFilters filters,
                                                         final CriteriaBuilder criteriaBuilder,
                                                         final Root<UserAccount> root,
                                                         final Collection<Predicate> predicates) {
        if (filters.getDepartment() == null) return;
        Join<UserAccount, Department> departmentJoin = root.join(DEPARTMENT_FIELD_NAME);
        Predicate userDepartmentEqualPredicate = criteriaBuilder.equal(departmentJoin.get("id"), filters.getDepartment());
        predicates.add(userDepartmentEqualPredicate);
    }
